...
from ssv_agent.models.event import DetectionEvent
...
class AgentService:
    ...
    def __init__(self, config: SsvConfig, mock_provider: bool = True) -> None:
        self._config = config
        self._running = False
        ...
        self._writer = ResultWriter(
            redis_client=self._consumer.redis_client,
            stream_key=f"{config.redis.stream_key}:reviews",
        )
        ...
        # 集成接口（当前全部 mock，T1/T2/T3 接通后切换）
        self._evidence_provider = MockEvidenceProvider()
        ...
        # provider（当前阶段 mock 先行）
        provider = MockProvider() if mock_provider else None

        self._machine = StateMachine(
            provider=provider,
            tool_router=None,  # M4 阶段工具路由暂未接入
