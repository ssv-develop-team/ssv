...
# file under .deps; it never copies or mutates the user's headers or libraries.
...
ssv_opencv_local_prepare() {
    ...
    local root="$SSV_ROOT/.deps/opencv-local"
    local pc_dir="$root/lib/pkgconfig"
    local candidate
    candidate="$(ssv_deps_make_candidate_dir "$root" opencv-local)"
    mkdir -p -- "$candidate/lib/pkgconfig"
    ssv_opencv_make_pc "$candidate" "$expected_version" "$include_dir" "$lib_dir"
    # The generated pc is the only managed artifact. Publish it atomically so
    # a failed probe cannot leave a partially written dependency source.
    local old_pkg_config_path="${PKG_CONFIG_PATH:-}"
    export PKG_CONFIG_PATH="$candidate/lib/pkgconfig${old_pkg_config_path:+:$old_pkg_config_path}"
    local runtime_dirs
