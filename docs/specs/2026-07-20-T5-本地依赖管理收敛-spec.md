...
- `SOURCE` 回答依赖来源。ONNX Runtime 和 TensorRT 使用 `managed|system`；OpenCV 由 `2026-07-21-T5-OpenCV-local-provider-spec.md` 扩展为 `managed|local|system`。
...
local 来源要求用户提供本机编译的 OpenCV `4.10.0` 头文件和动态库目录，完整契约见 `2026-07-21-T5-OpenCV-local-provider-spec.md`。system 来源最低支持版本为 `4.5`。disabled 模式完全不准备、不发现、不探测 OpenCV。
